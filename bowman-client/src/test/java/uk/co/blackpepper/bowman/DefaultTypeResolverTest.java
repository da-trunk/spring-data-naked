...
public class DefaultTypeResolverTest {
  ...
  public static class TypeWithNonRemoteResourceSubtypeSubtype
      extends TypeWithNonRemoteResourceSubtype {
    // no members
  }
  ...
  public void resolveTypeWithNoResourceTypeInfoReturnsDeclaredType() {
    Class<?> type =
        resolver.resolveType(
            TypeWithoutInfo.class,
            Links.of(Link.of("http://x", IanaLinkRelations.SELF)),
            Configuration.build());
  ...
  public void resolveTypeWithSubtypesAndMatchingAbsoluteUriSubtypeReturnsSubtype() {
    ...
    Class<?> type =
        resolver.resolveType(
            TypeWithSubtypes.class,
            Links.of(Link.of("http://x.com/2/1", IanaLinkRelations.SELF)),
            config);
  ...
  public void resolveTypeWithSubtypesAndMatchingAbsolutePathReferenceUriSubtypeReturnsSubtype() {
    ...
    Class<?> type =
        resolver.resolveType(
            TypeWithSubtypes.class, Links.of(Link.of("/2/1", IanaLinkRelations.SELF)), config);
  ...
  public void resolveTypeWithNonRemoteResourceSubtypeThrowsException() {
    ...
    thrown.expectMessage(
        TypeWithNonRemoteResourceSubtypeSubtype.class.getName()
            + " is not annotated with @RemoteResource");

    resolver.resolveType(
        TypeWithNonRemoteResourceSubtype.class,
        Links.of(Link.of("/", IanaLinkRelations.SELF)),
        Configuration.build());
  }
  ...
  public void resolveTypeWithIllegalSubtypeThrowsException() {
    thrown.expect(ClientProxyException.class);
    thrown.expectMessage(
        TypeWithIllegalSubtypeSubtype.class.getName()
            + " is not a subtype of "
            + TypeWithIllegalSubtype.class.getName());

    resolver.resolveType(
        TypeWithIllegalSubtype.class,
        Links.of(Link.of("/x/1", IanaLinkRelations.SELF)),
        Configuration.build());
  }
  ...
  public void resolveTypeWithResolverReturnsResolvedType() {
    ...
    doReturn(TypeWithResolverSubtype.class)
        .when(Resolver.mockResolver)
        .resolveType(TypeWithResolver.class, links, config);
