...
public class CEClient<T extends WithUri & WithId<?>> extends Client<T> {
  ...
  protected CEClient(
      Class<T> entityType,
      String batchPath,
      Configuration bowmanConfiguration,
      RestOperations restOperations,
      ClientProxyFactory proxyFactory) {
    this(
        entityType,
        UriComponentsBuilder.fromUri(bowmanConfiguration.getBaseUri())
            .path(batchPath)
            .build()
            .toUri(),
        bowmanConfiguration,
  ...
  public T search(String path, String name, Object val) {
    URI uri =
        UriComponentsBuilder.fromUri(getBaseUri())
  ...
  public CollectionModel<EntityModel<T>> saveAll(CollectionDTO<T> wrapped) {
    ...
    JavaType innerType =
        objectMapper.getTypeFactory().constructParametricType(EntityModel.class, entityType);
    JavaType targetType =
        objectMapper.getTypeFactory().constructParametricType(CollectionModel.class, innerType);
  ...
  private static class MyObjectMapperFactory extends DefaultObjectMapperFactory {
    ...
    public ObjectMapper create(HandlerInstantiator instantiator) {
      return super.create(instantiator)
  ...
  public static class Factory {
    ...
    private final RestTemplateFactory restTemplateFactory = new DefaultRestTemplateFactory();
    private final ObjectMapperFactory objectMapperFactory = new MyObjectMapperFactory();
    ...
    public Factory(final ClientProperties properties) {
      ...
      restOperations =
          new RestOperationsFactory(
                  configuration, proxyFactory, objectMapperFactory, restTemplateFactory)
              .create();
    }
    ...
    public <T extends WithUri & WithId<?>> CEClient<T> create(Class<T> entityType) {
      return new CEClient<>(
          entityType, properties.getBatchPath(), configuration, restOperations, proxyFactory);
    }
  }
}
