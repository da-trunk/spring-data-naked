...
public class CEClient<T extends WithUri & WithId<?>> extends Client<T> {
  ...
  protected CEClient(Class<T> entityType, String batchPath, Configuration bowmanConfiguration, RestOperations restOperations,
    ...
    this(entityType, UriComponentsBuilder.fromUri(bowmanConfiguration.getBaseUri()).path(batchPath).build().toUri(), bowmanConfiguration,
        restOperations, proxyFactory);
  }
  ...
  public <O extends WithUri & WithId<?>> CEClient<O> create(Class<O> entityType) {
    return new CEClient<O>(entityType, batchUri, getConfiguration(), getRestOperations(), getProxyFactory());
  }
  ...
  public Iterable<T> findAll() {
    URI uri = UriComponentsBuilder.fromUri(getBaseUri()).pathSegment("search", "findAll").build().toUri();
  ...
  public T search(String path, String name, Object val) {
    URI uri = UriComponentsBuilder.fromUri(getBaseUri()).pathSegment("search", path).queryParam(name, val).build().toUri();
    return get(uri);
  }
  ...
  protected URI getEntityUri(T entity) {
    URI uri = UriComponentsBuilder.fromUri(getBaseUri()).pathSegment(entity.getId().toString()).build().toUri();
  ...
  /**
   * POST the provided {@link CollectionDTO}-wrapped collection to
   * {@link #batchUri}.
   *
  ...
  public CollectionModel<EntityModel<T>> saveAll(CollectionDTO<T> wrapped) {
    ...
    try {
      node = restOperations.getRestTemplate().postForObject(batchUri, wrapped, ObjectNode.class);
  ...
  private static class MyObjectMapperFactory extends DefaultObjectMapperFactory {
    @Override
    public ObjectMapper create(HandlerInstantiator instantiator) {
      return super.create(instantiator).registerModule(new JavaTimeModule())
  ...
  public static class Factory {
    ...
    private ObjectMapperFactory objectMapperFactory = new MyObjectMapperFactory();
    private final ClientProperties properties;

    public Factory(final ClientProperties properties) {
      this.properties = properties;
      this.configuration = uk.co.blackpepper.bowman.Configuration.builder().setBaseUri(properties.getLocation()).build();
