...
/**
 * Class for retrieving, persisting and deleting annotated entity instances via remote hal+json
 * resources.
 *
 * <p>Entities can contain simple (directly mappable to JSON) properties, and inline or linked
 * associations to further objects.
 *
...
public class Client<T> {
  ...
  /**
   * GET a single entity from the entity's base resource (determined by the class's {@link
   * uk.co.blackpepper.bowman.annotation.RemoteResource} annotation).
   *
  ...
  /**
   * GET a collection of entities from the entity's base resource (determined by the class's {@link
   * uk.co.blackpepper.bowman.annotation.RemoteResource} annotation).
   *
  ...
  public Iterable<T> getAll(URI uri) {
    ...
    for (EntityModel<T> resource : resources) {
      T val = proxyFactory.create(resource, restOperations);
      result.add(val);
    }
  ...
  /**
   * POST the given entity to the entity's base resource (determined by the class's {@link
   * uk.co.blackpepper.bowman.annotation.RemoteResource} annotation).
   *
   * <p>The entity will be updated with the URI ID the remote service has assigned it.
   *
  ...
  /**
   * PUT the given entity to the entity's base resource (determined by the class's {@link
   * uk.co.blackpepper.bowman.annotation.RemoteResource} annotation).
   *
   * @param object the entity to submit
   */
