 ...
 *
 * <p>If OAuth properties are included in the properties passed to the constuctor, then every
 * request made through this client will acquire a OAuth token if necessary and attach OAuth headers
 * to the request. If OAuth properties are not provided, these headers will not be attached.
 *
 * <p>TODO: consider making this an interface and splitting implementations into separate classes
 * (one for PagingClient and the other for StreamingClient).
 *
...
public class FunctionalClient {
  ...
  @Getter final long[] retrySleepDurations;
  ...
  /**
   * Override to determine whether we retry, given the HttpStatus associated with a server's
   * response.
   *
   ...
   * @param supplier a ResponseEntitySupplier. This should return a {@code ResponseEntity<? extends
   *     T>} each time we retry.
   * @return {@code Page<T>} the response
   * @throws IOException on a non-retryable error or after max retry attempts.
   */
  public <T> T executeWithRetry(URI uri, ResponseEntitySupplier<? extends T> supplier)
      throws IOException {
    int attempt = 1;
    ...
    do {
      try {
        ...
        log.debug(
            ...
            Duration.between(startTime, java.time.Instant.now()).toMillis());
        if (response.getHeaders().containsKey("Location")) {
          log.debug("Response locations [{}]", response.getHeaders().get("Location"));
        }
        return response.getBody();
      } catch (HttpStatusCodeException e) {
        cause = e;
        // If RestTemplate throws this exception, it will include the response body. See
        // e.getResponseBodyAsString().
        log.warn(
            "Request [{}] failed with status [{}]: {}, {}", uri, e.getStatusCode(), e.getMessage());
        log.warn("Response: {}", e.getResponseBodyAsString());
        if (!canRetry(e.getStatusCode()) && !canRetry(e)) throw new IOException(e);
        ...
        if (wrapped.getCause() != null && wrapped.getCause() instanceof IOException) {
          ...
          if (log.isDebugEnabled()) log.catching(Level.DEBUG, wrapped);
          else log.warn(wrapped.getMessage());
          throw wrapped;
        }
      } catch (IOException e) {
        cause = e;
      ...
      try {
        log.warn(
            "executeWithRetry: initiating attempt [{}] after sleeping for [{}] seconds",
            attempt,
            sleepInterval);
        if (sleepInterval > 0) {
