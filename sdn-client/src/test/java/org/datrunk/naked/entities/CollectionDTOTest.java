...
@SpringBootTest(
    classes = CollectionDTOTest.Config.class,
...
public class CollectionDTOTest {
  ...
  public void testDeserializationWithoutTypeRef()
      throws JsonMappingException, JsonProcessingException {
    String baseUri = "http://bogus";
    final RestOperationsInstantiation restOperationsFactory =
        new RestOperationsInstantiation("http://bogus");
    ObjectMapper objectMapper = restOperationsFactory.getObjectMapper();

    String parentPath = Parent.class.getAnnotation(RemoteResource.class).value();
    URI parentBaseUri =
        UriComponentsBuilder.fromUri(URI.create(baseUri)).path(parentPath).build().toUri();

    Parent homer = Parent.of("Homer");
    RestTemplate mockRestTemplate = Mockito.mock(RestTemplate.class);
    Mockito.when(mockRestTemplate.postForLocation(parentBaseUri, homer))
        .thenReturn(UriComponentsBuilder.fromUri(parentBaseUri).path("/homer").build().toUri());
    {
  ...
  private static class RestOperationsInstantiation extends HandlerInstantiator {
    @Getter private final RestOperations restOperations;
    ...
    public RestOperationsInstantiation(String baseUrl) {
      configuration = uk.co.blackpepper.bowman.Configuration.builder().setBaseUri(baseUrl).build();
    ...
    static class InlineAssociationDeserializer<T> extends StdDeserializer<T>
      ...
      InlineAssociationDeserializer(
          Class<T> type, RestOperations restOperations, ClientProxyFactory proxyFactory) {
        super(type);
        this.type = type;
      ...
      public T deserialize(JsonParser p, DeserializationContext ctxt)
        ...
        JavaType resourceType =
            ctxt.getTypeFactory().constructParametricType(EntityModel.class, type);
      ...
      public JsonDeserializer<?> createContextual(
        ...
        return new InlineAssociationDeserializer<>(
            ctxt.getContextualType().getRawClass(), restOperations, proxyFactory);
      }
    }

    static class ResourceDeserializer extends StdDeserializer<EntityModel<?>>
        implements ContextualDeserializer {
      ...
      public JsonDeserializer<?> createContextual(
        ...
        Class<?> resourceContentType =
            ctxt.getContextualType().getBindings().getTypeParameters().get(0).getRawClass();

        return new ResourceDeserializer(resourceContentType, typeResolver, configuration);
      }

      @Override
      public EntityModel<?> deserialize(JsonParser p, DeserializationContext ctxt)
          throws IOException {
      ...
      private Class<?> getResourceDeserializationType(Links links) {
        Class<?> resourceContentType =
            typeResolver.resolveType(handledType(), links, configuration);
  ...
  static class DefaultTypeResolver implements TypeResolver {
    @Override
    public <T> Class<? extends T> resolveType(
        Class<T> declaredType, Links resourceLinks, Configuration config) {
      ResourceTypeInfo info = AnnotationUtils.findAnnotation(declaredType, ResourceTypeInfo.class);
      if (info == null) {
        return declaredType;
      }
      boolean customTypeResolverIsSpecified =
          info.typeResolver() != ResourceTypeInfo.NullTypeResolver.class;
  ...
  static class SelfLinkTypeResolver implements TypeResolver {
    ...
    @Override
    public <T> Class<? extends T> resolveType(
        Class<T> declaredType, Links resourceLinks, Configuration configuration) {
      ...
      for (Class<?> candidateClass : subtypes) {
        RemoteResource candidateClassInfo =
            AnnotationUtils.findAnnotation(candidateClass, RemoteResource.class);

        if (candidateClassInfo == null) {
          throw new ClientProxyException(
              String.format(
                  "%s is not annotated with @%s",
                  candidateClass.getName(), RemoteResource.class.getSimpleName()));
        }

        String resourcePath = candidateClassInfo.value();

        String resourceBaseUriString =
            UriComponentsBuilder.fromUri(configuration.getBaseUri())
                .path(resourcePath)
                .toUriString();

        String selfLinkUriString =
            toAbsoluteUriString(self.get().getHref(), configuration.getBaseUri());

        if (selfLinkUriString.startsWith(resourceBaseUriString + "/")) {
          if (!declaredType.isAssignableFrom(candidateClass)) {
            throw new ClientProxyException(
                String.format(
                    "%s is not a subtype of %s", candidateClass.getName(), declaredType.getName()));
          }
    ...
    private static String toAbsoluteUriString(String uri, URI baseUri) {
      ...
      return UriComponentsBuilder.fromUri(baseUri).path(uri).toUriString();
    }
  }
}
