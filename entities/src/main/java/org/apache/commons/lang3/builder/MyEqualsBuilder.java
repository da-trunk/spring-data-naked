 ...
 *
 * <p>
 * This class provides methods to build a good equals method for any class. It follows rules laid out in
 * <a href="http://www.oracle.com/technetwork/java/effectivejava-136174.html">Effective Java</a> , by Joshua Bloch. In
 * particular the rule for comparing {@code doubles}, {@code floats}, and arrays can be tricky. Also, making sure that
 * {@code equals()} and {@code hashCode()} are consistent can be difficult.
 * </p>
 *
 * <p>
 ...
 * usually private, the method, {@code reflectionEquals}, uses {@code AccessibleObject.setAccessible} to change the
 * visibility of the fields. This will fail under a security manager, unless the appropriate permissions are set up
 * correctly. It is also slower than testing explicitly. Non-primitive fields are compared using {@code equals()}.
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * The {@link EqualsExclude} annotation can be used to exclude fields from being used by the {@code reflectionEquals}
 * methods.
 * </p>
 *
 * @since 1.0
...
public class MyEqualsBuilder implements org.apache.commons.lang3.builder.Builder<Boolean> {
	 ...
	 * @since 3.0
	 */
	private static final ThreadLocal<Set<Pair<org.apache.commons.lang3.builder.IDKey, IDKey>>> REGISTRY =
		new ThreadLocal<>();
	...
	/**
	 * <p>
	 * Returns {@code true} if the registry contains the given object pair. Used by the reflection methods to avoid
	...
	/**
	 * Set whether to test fields recursively, instead of using their equals method, when reflectively comparing
	 * objects. String objects, which cache a hash value, are automatically excluded from recursive testing. You may
	 * specify other exceptions by calling {@link #setBypassReflectionClasses(List)}.
	 * 
	...
	/**
	 * <p>
	 * Set {@code Class}es whose instances should be compared by calling their {@code equals} although being in
	 * recursive mode. So the fields of theses classes will not be compared recursively by reflection.
	 * </p>
	 *
	 * <p>
	 * Here you should name classes having non-transient fields which are cache fields being set lazily.<br>
	 * Prominent example being {@link String} class with its hash code cache field. Due to the importance of the
	 * {@code String} class, it is included in the default bypasses classes. Usually, if you use your own set of classes
	 * here, remember to include {@code String} class, too.
	 * </p>
	 * 
	 ...
	 *
	 * <p>
	 * It uses {@code AccessibleObject.setAccessible} to gain access to private fields. This means that it will throw a
	 * security exception if run under a security manager, if the permissions are not set up correctly. It is also not
	 * as efficient as testing explicitly. Non-primitive fields are compared using {@code equals()}.
	 * </p>
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * If the testTransients parameter is set to {@code true}, transient members will be tested, otherwise they are
	 * ignored, as they are likely derived fields, and not part of the value of the {@code Object}.
	 * </p>
	 *
	 * <p>
	 * Static fields will not be included. Superclass fields will be appended up to and including the specified
	 ...
	 *
	 * <p>
	 * If the testRecursive parameter is set to {@code true}, non primitive (and non primitive wrapper) field types will
	 * be compared by {@code EqualsBuilder} recursively instead of invoking their {@code equals()} method. Leading to a
	 * deep reflection equals test.
	 *
	 * @param lhs {@code this} object
	 * @param rhs the other object
	 ...
	 * @since 3.6
	 */
	public static boolean reflectionEquals(final Object lhs, final Object rhs, final Class<?> reflectUpToClass,
		...
		return new MyEqualsBuilder().setExcludeFields(excludeFields)
	 ...
	 *
	 * <p>
	 * If the testTransients field is set to {@code true}, transient members will be tested, otherwise they are ignored,
	 * as they are likely derived fields, and not part of the value of the {@code Object}.
	 * </p>
	 *
	 * <p>
	 * Static fields will not be included. Superclass fields will be appended up to and including the specified
	 * superclass in field {@code reflectUpToClass}. A null superclass is treated as java.lang.Object.
	 * </p>
	 *
	 * <p>
	 ...
	 *
	 * <p>
	 * If either class of the compared objects is contained in {@code bypassReflectionClasses}, both objects are
	 * compared by calling the equals method of the left hand object with the right hand object as an argument.
	 * </p>
	 *
	...
	public MyEqualsBuilder reflectionAppend(final Object lhs, final Object rhs)
		...
		if (bypassReflectionClasses != null
			&& (bypassReflectionClasses.contains(testClass) || bypassReflectionClasses.contains(child.getClass()))) {
			...
			for (Method method : clazz.getDeclaredMethods()) {
				...
				if (method.getName()
					...
					&& !method.getReturnType()
						.equals(Void.TYPE)
					&& !ArrayUtils.contains(excludeFields, method.getName()) && !Modifier.isTransient(modifiers)
	...
	/**
	 * <p>
	 * Test if two {@code Object}s are equal using either #{@link #reflectionAppend(Object, Object)}, if object are non
	 * primitives (or wrapper of primitives) or if field {@code testRecursive} is set to {@code false}. Otherwise, using
	 * their {@code equals} method.
	 * </p>
	 *
	 * @param lhs the left hand object
	 * @param rhs the right hand object
	...
	/**
	 * <p>
	 * Test if two {@code double}s are equal by testing that the pattern of bits returned by {@code doubleToLong} are
	 * equal.
	 * </p>
	 *
	 * <p>
	 ...
	 * </p>
	 *
	 * @return {@code true} if all of the fields that have been checked are equal, {@code false} otherwise.
	 *
	 * @since 3.0
	 */
	@Override
