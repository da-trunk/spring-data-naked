...
public class LinkedResourceMethodHandlerTest {
  ...
  public void setUp() {
    ...
    handler =
        new LinkedResourceMethodHandler(
            resource,
            mock(RestOperations.class),
            proxyFactory,
            propertyValueFactory,
            methodLinkAttributesResolver,
            methodLinkUriResolver);

    when(methodLinkAttributesResolver.resolveForMethod(any()))
        .thenReturn(newRequiredLinkAttributes());
  }
  ...
  public void supportsSetterForAnnotatedGetterIsTrue() {
    assertThat(
        handler.supports(findMethod(ResourceContent.class, "setLinkedResource", String.class)),
        is(true));
  }
  ...
  public void supportsNonAnnotatedMethodIsFalse() {
    assertThat(
        handler.supports(findMethod(ResourceContent.class, "nonAnnotatedMethod")), is(false));
  }

  @Test
  public void invokeResolvesLinkFromLinkAttributes()
      throws InvocationTargetException, IllegalAccessException {
    final Method getterMethod = findMethod(ResourceContent.class, "getLinkedResource");

    when(methodLinkAttributesResolver.resolveForMethod(getterMethod))
        .thenReturn(new MethodLinkAttributes("x", true));

    handler.invoke(resourceContent, getterMethod, null, new String[0]);

    verify(methodLinkUriResolver)
        .resolveForMethod(eq(resource), eq("x"), argThat(is(emptyArray())));
  }

  @Test
  public void invokeWhenNoLinkAndRequiredLinkThrowsException()
      throws InvocationTargetException, IllegalAccessException {
    ...
    when(methodLinkAttributesResolver.resolveForMethod(any()))
        .thenReturn(newRequiredLinkAttributes());
    when(methodLinkUriResolver.resolveForMethod(any(), any(), any()))
  ...
  @Test
  public void invokeWhenNoLinkAndOptionalLinkAndSingleValueReturnsNull()
      throws InvocationTargetException, IllegalAccessException {

    final Method getterMethod = findMethod(ResourceContent.class, "getOptionalLinkedResource");

    when(methodLinkAttributesResolver.resolveForMethod(any()))
        .thenReturn(newOptionalLinkAttributes());
    when(methodLinkUriResolver.resolveForMethod(any(), any(), any()))
        .thenThrow(new NoSuchLinkException("x"));
  ...
  @Test
  public void invokeWhenNoLinkAndOptionalLinkAndCollectionValueReturnsEmpty()
      throws InvocationTargetException, IllegalAccessException {
  ...
  @Test
  public void invokeSetsAndReturnsSameLinkedResource()
      throws InvocationTargetException, IllegalAccessException {
    final Method getterMethod = findMethod(ResourceContent.class, "getLinkedResource");
    final Method setterMethod =
        findMethod(ResourceContent.class, "setLinkedResource", String.class);
