...
public class BatchRestRepo {
    ...
    public ResponseEntity<CollectionModel<EntityModel<? extends WithId>>> saveAll(
        ...
        List<? extends WithId> entities = Objects.requireNonNull(dto.getContent())
            .getEntities();
        List<? extends WithId> mergedEntities = entities.stream()
            .map(em::merge)
            .collect(Collectors.toList());
        em.flush();

        ArrayList<EntityModel<? extends WithId>> resources = mergedEntities.stream()
            .map(entity -> {
                Link selfLink = entityLinks.linkToItemResource(entity.getClass(), entity.getId())
                    .withSelfRel();
                EntityModel<? extends WithId> resource = EntityModel.of(entity)
                    .add(selfLink);
                List<Link> links = Stream.of(entity.getClass()
                    .getMethods())
    ...
    private static boolean isEntityGetter(Method method) {
        ...
        if (Collection.class.isAssignableFrom(method.getReturnType())) {
            ...
            if (genericReturnType instanceof ParameterizedType) {
                Type type = ((java.lang.reflect.ParameterizedType) genericReturnType).getActualTypeArguments()[0];
        ...
        if (possibleEntityType.getAnnotation(Entity.class) == null)
            return false;
        return true;
    }
    ...
    private static Class<?> getEntityTypeFromProxy(Object proxy) {
        Class<?> parentType = proxy.getClass()
            .getSuperclass();
    ...
    public ResponseEntity<List<?>> persist(@RequestBody EntityModel<CollectionDTO<?>> dto) throws IOException {
        List<?> entities = Objects.requireNonNull(dto.getContent())
            .getEntities();
        entities.stream()
            .forEach(em::persist);
        em.flush();
